...
function DearEmocGui.CreateWindow(title, subtitle)
    ...
    -- Sekme Oluşturma Fonksiyon
        -- Element Oluşturma Fonksiyonları
        -- Sekme Oluşturma Fonksiyonu (DÜZELTİLMİŞ)
function Window.CreateTab(tabName, tabIcon)
    ...
    TabButton.Size = isMobile and UDim2.new(1, -10, 0, 40) or UDim2.new(0, 110, 0, 38)
    TabButton.Font = Enum.Font.GothamBold
    TabButton.Text = tabIcon and (tabIcon .. " " .. tabName) or tabName
    TabButton.TextColor3 = ColorPalette.Text
    TabButton.TextSize = isMobile and 15 or 16
    TabButton.TextTransparency = 0.3
    TabButton.Parent = TabContainer
    
    -- Yuvarlak köşeler
    ApplyRoundedCorners(TabButton, 6)
    
    -- Hover efekti (sadece PC'de)
    if not isMobile then
        TabButton.MouseEnter:Connect(function()
            if activeTab ~= TabButton then
                Tween(TabButton, {BackgroundColor3 = ColorPalette.TabHover})
            end
        end)
        
        TabButton.MouseLeave:Connect(function()
            if activeTab ~= TabButton then
                Tween(TabButton, {BackgroundColor3 = ColorPalette.Element})
            end
        end)
    end
    ...
    TabContent.Visible = false -- Başlangıçta tüm sekmeler gizli
    ...
    if #Tabs == 0 then
        activeTab = TabButton
        TabButton.BackgroundColor3 = ColorPalette.TabActive
        TabButton.TextTransparency = 0
        TabContent.Visible = true
    end
    
    -- Sekme Değiştirme (TAM DÜZELTME)
    TabButton.MouseButton1Click:Connect(function()
        -- Eğer zaten bu sekmedeyse, hiçbir şey yapma
        ...
        if activeTab then
            activeTab.BackgroundColor3 = ColorPalette.Element
            activeTab.TextTransparency = 0.3
        end
        ...
        -- Aktif sekme değişkenini güncelle
        activeTab = TabButton
    end)
