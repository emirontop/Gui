--[[
    DearEmocGui v6.1 - Toggle Renk Sorunu Çözümlü
    Düzeltmeler:
        - Toggle'ların renk güncelleme sorunu çözüldü
        - Toggle state'leri artık doğru şekilde güncelleniyor
        - Gradient uygulaması optimize edildi
        - Mobil için küçük iyileştirmeler
]]
...
-- Gradient Efekti Fonksiyonu (Güncellendi)
local function ApplyGradient(obj, color1, color2, rotation)
    -- Önceki gradient'i temizle
...
function DearEmocGui.CreateWindow(title, subtitle)
    ...
    function Window.CreateTab(tabName)
        ...
        function Tab.CreateToggle(togName, default, callback)
            ...
            -- Toggle rengini güncelleme fonksiyonu
            local function UpdateToggleAppearance()
                local targetPosition = currentState and UDim2.new(0.5, 0, 0, 1) or UDim2.new(0, 1, 0, 1)
                local color1 = currentState and ColorPalette.Success or ColorPalette.Disabled
                local color2 = currentState and ColorPalette.Success:lerp(Color3.new(0, 0, 0), 0.2) or ColorPalette.Disabled:lerp(Color3.new(0, 0, 0), 0.2)
                
                Tween(ToggleButton, {Position = targetPosition}, 0.2, Enum.EasingStyle.Back)
                ApplyGradient(ToggleButton, color1, color2)
            end
            ...
            -- Toggle'ın görünümünü ilk duruma göre ayarla
            UpdateToggleAppearance()
